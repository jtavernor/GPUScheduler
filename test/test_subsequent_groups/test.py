...
def run_experiments(allow_crash=False):
    ...
    jobs = {f'script_name_{i}': {'model_config': '...', 'script id': i} for i in range(7)}
    print('launching jobs')
    scheduler.launch_jobs(jobs, allow_crash)
    print('Should have waited until all jobs finished')
    later_jobs = {f'post_script_name_{i}': {'model_config': '...', 'script id': i} for i in range(9)}
    scheduler.launch_jobs(later_jobs, allow_crash)
...
class ProcessRunAfter:
    def __call__(self, **args):
        print('PROCESS RUNNING AFTER ALL OTHER JOBS')
        print('job begun with args:', args)
        print('num gpus available to script:', torch.cuda.device_count())
        a = torch.randn((500,500), device='cuda')
        print('created cuda variable with mean:', a.mean())
